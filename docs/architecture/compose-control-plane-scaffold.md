...
| MQTT | `control` | `mqtt-data`; read-only config | none | event transport only |
...
- Use named volumes only for PostgreSQL, MQTT, and Hermes state. Do not use anonymous volumes.
...
- Commit only non-secret `compose.yaml`, `.env.example`, Caddy configuration, Mosquitto configuration template, and secret-file instructions.
...
- MQTT has anonymous access disabled and no device enrollment until its authentication and topic-ACL design is approved.
...
PostgreSQL health must pass before Jarvis API starts. MQTT health and Hermes readiness are finalized only after inspecting their approved image documentation; do not invent health commands. Jarvis API will later expose an internal redacted `/healthz`; Caddy proxies user routes only after that signal passes.
...
MQTT credential/topic ACLs, Hermes-to-Jarvis authorization, exact image commands and health signals, at-rest encryption implementation, Jarvis API implementation, dashboard routes, model routing, OAuth, observability, and external access are separate work.
